 ...
 * Simple Queries
 */
select * from jbearden.Album;
select * from jbearden.Album order by year desc;
select * from jbearden.Band where name like 'The %';
select * from jbearden.Song order by title;
select(albumId) from jbearden.Album where title = 'Nevermind';

/*
 * Simple Aggregate Queries
 */
select count(*) from jbearden.Musician;
select * from jbearden.Album a where a.year < (select avg(year) from jbearden.Album);
select sum(trackLength) from jbearden.AlbumSong where albumId = (select(albumId) from jbearden.Album where title = 'Rain Dogs');

/*
 * Join Queries
 */
select a.title, a.year, b.name from jbearden.Album a join jbearden.Band b on a.bandId = b.bandId;
select a.title as albumTitle, s.title as songTitle from jbearden.Album a join jbearden.AlbumSong n on a.albumId = n.albumId join jbearden.Song s on n.songId = s.songId join jbearden.Band b on a.bandId = b.bandId where b.name = 'Nirvana';
select * from jbearden.Band b left join jbearden.Album a on b.bandId = a.bandId;
